...
class TaobaoAdapter(PlatformAdapter):
	"""Taobao/千牛 merchant backend adapter for price management tasks.
	...
	LOGIN_URL = "https://myseller.taobao.com/"

	def get_prepare_system_prompt(self, instruction: str) -> str:
		"""Return the prepare-phase system prompt for Taobao price changes.
		...
		"""
		return f"""你是一个淘宝/千牛商家后台的自动化操作助手。

你的任务是准备一次商品价格修改操作。请严格按照以下步骤执行：

## 步骤 1：确认登录状态
- 当前页面应为淘宝商家中心（https://myseller.taobao.com/）或千牛工作台
- 如果页面显示登录界面，请停下来并报告需要登录，不要尝试自动登录
- 如果已登录，继续下一步

## 步骤 2：导航到商品管理
- 找到左侧菜单栏中的「商品」或「商品管理」选项并点击
- 在子菜单中选择「出售中的宝贝」或「商品列表」
- 等待商品列表页面加载完成

## 步骤 3：找到目标商品
- 根据指令中的商品名称，在商品列表中搜索或浏览找到目标商品
- 如果有搜索框，可以使用搜索功能快速定位
- 找到商品后，点击该商品的「编辑商品」按钮进入编辑页面

## 步骤 4：修改价格
- 在商品编辑页面中，找到「一口价」或「销售价格」输入字段
- 清除当前价格值
- 输入目标价格值：{instruction}
- 确认价格已正确填入输入框
...
"""
	...
	def get_execute_system_prompt(self, instruction: str) -> str:
		"""Return the execute-phase system prompt for Taobao price changes.
...
"""
	...
	def build_task_prompt(self, instruction: str, params: dict) -> str:
		"""Build the full task prompt for a Taobao price change operation.
