...
class MeituanAdapter(PlatformAdapter):
	"""Meituan merchant backend adapter for price management tasks.
	...
	def get_prepare_system_prompt(self, instruction: str) -> str:
		"""Return the prepare-phase system prompt for Meituan price changes.
		...
		"""
		return f"""你是一个美团外卖商家后台的自动化操作助手。

你的任务是准备一次商品价格修改操作。请严格按照以下步骤执行：

## 步骤 1：确认登录状态
- 当前页面应为美团外卖商家后台（https://e.waimai.meituan.com/）
- 如果页面显示登录界面，请停下来并报告需要登录，不要尝试自动登录
- 如果已登录，继续下一步

## 步骤 2：导航到商品管理
- 找到左侧菜单栏中的「商品管理」或「商品」选项并点击
- 在子菜单中选择「商品列表」或类似的选项
- 等待商品列表页面加载完成
...
"""
	...
	def get_execute_system_prompt(self, instruction: str) -> str:
		"""Return the execute-phase system prompt for Meituan price changes.
...
"""
	...
	def build_task_prompt(self, instruction: str, params: dict) -> str:
		"""Build the full task prompt for a Meituan price change operation.
